...
> Preimport(ctx).XAPITOKEN(xAPITOKEN).XRequestedWith(xRequestedWith).Include(include).Execute()
...
func main() {
	...
	r, err := apiClient.ImportsAPI.Preimport(context.Background()).XAPITOKEN(xAPITOKEN).XRequestedWith(xRequestedWith).Include(include).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ImportsAPI.Preimport``: %v\n", err)
...
Other parameters are passed through a pointer to a apiPreimportRequest struct via the builder pattern
