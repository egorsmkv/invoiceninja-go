...
> DeletePaymentTerm(ctx, id).XAPITOKEN(xAPITOKEN).XRequestedWith(xRequestedWith).Include(include).Execute()
...
func main() {
	...
	r, err := apiClient.PaymentTermssAPI.DeletePaymentTerm(context.Background(), id).XAPITOKEN(xAPITOKEN).XRequestedWith(xRequestedWith).Include(include).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PaymentTermssAPI.DeletePaymentTerm``: %v\n", err)
...
Other parameters are passed through a pointer to a apiDeletePaymentTermRequest struct via the builder pattern
