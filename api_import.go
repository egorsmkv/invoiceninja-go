...
type ApiGetImportJsonRequest struct {
	ctx context.Context
...
func (r ApiGetImportJsonRequest) XRequestedWith(xRequestedWith string) ApiGetImportJsonRequest {
	r.xRequestedWith = &xRequestedWith
	return r
}

func (r ApiGetImportJsonRequest) Execute() (*http.Response, error) {
	return r.ApiService.GetImportJsonExecute(r)
}

/*
GetImportJson Import data from the system

Import data from the system

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiGetImportJsonRequest
*/
func (a *ImportAPIService) GetImportJson(ctx context.Context) ApiGetImportJsonRequest {
	return ApiGetImportJsonRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *ImportAPIService) GetImportJsonExecute(r ApiGetImportJsonRequest) (*http.Response, error) {
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ImportAPIService.GetImportJson")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/import_json"
